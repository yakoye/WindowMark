...
以前：鼠标命中哪个标签，哪个就展开成「整个名字塞进标签」的长条，其余收起；鼠标滑过两个
标签之间的间隙，整条栏先塌下去再弹起来——间隙像素是全透明的，鼠标从那里直接落到宿主窗口上，
书签条以为鼠标走了。

现在：鼠标是一块磁铁，整条书签栏是一排铁块。

- 每个标签按它离鼠标的距离（沿书签栏方向）连续放大：正对标签中心时长到固定峰值——绝对像素，
  不是倍率，所以激活标签和普通标签被吸到时一样大——`drawer.magnet_radius`（默认 120px）以外
  完全不动，中间按余弦平滑衰减。
- 放大的标签把邻居连续挤开，鼠标下的那一点始终留在鼠标下，标签不会跑开让鼠标去追。
- 间隙不再是死区：间隙里画一层 alpha = 1/255 的「包络带」，肉眼看不见，但接得住鼠标。停在两个
  标签正中间时两边一样大。
- 进出整条栏时磁场强度渐变（`drawer.animation_ms`），离开后先等 `drawer.magnet_grace_ms`（默认
  60ms）宽限，回来得及时就当没离开过。栏内移动时磁场即时跟着鼠标，只有约一帧的轻微平滑。
- 鼠标只和 base（静止时的位置）比距离，画出来的样子从不参与下一次计算，不会越放越大。

### 书签 → 浮动标题 → 缩略图，三段依次排开

- 标签里只放短名，完整名字在独立的浮动标题里，缩略图在标题外侧。从窗口边缘往窗口内容方向永远
  是 书签 → 标题 → 缩略图，任何时刻互不重叠：标题躲开它下方**所有**标签里最深的那个，而不只是
  主标签，所以邻居长高时标题和缩略图被连续推开，间距保持 `preview.title_gap`（7px）和
  `preview.thumbnail_gap`（10px）。
- 左右两侧的标题是真正的竖排排版（中文直立、英文侧转），两侧都从上往下读。
- 标题超长时末尾省略号：横排不超过缩略图宽，竖排不超过缩略图高。
- 主标签换人时，标题和缩略图在 `preview.crossfade_ms`（默认 100ms）里从旧位置滑到新位置，内容
  交叉淡入淡出，没有空白帧。鼠标停在两个标签正中间抖动时有滞回，标题不来回闪。
- 缩略图第一次出现仍等 `preview.delay_ms`，之后跟着主标签换，不再等。空间不够时按比例缩小，
  小于 120×80 就不显示，绝不压到标题或书签上。
- `preview.enabled` 现在只管缩略图，浮动标题总是显示——标签里只放得下几个字。
- 书签指向当前窗口自己时没有缩略图可放，缩略图的位置上改显示大字「当前窗口」，和别的书签
  之间照样交叉淡入淡出，版面不跳。

### 书签右键菜单的「重命名」「设置」点了没反应

菜单弹得出来，点上面的项却什么都不发生。菜单的主人必须是前台窗口，菜单项才点得动——托盘菜单
一直是这么做的；书签条却是 `WS_EX_NOACTIVATE` 的、当不了前台，以前的做法是把前台交给宿主窗口，
可宿主在别的进程里，菜单所属的线程于是不是前台线程。

改成由本进程里一个看不见、能激活的窗口当菜单的主人，弹菜单前把前台给它，和托盘同一套做法。
前台临时落到本进程的隐藏窗口上，窗口跟踪看不到（事件钩子跳过本进程），宿主仍被当作活动窗口，
书签条和边框都不会闪。菜单没选任何项时把前台还给宿主；选了的话，命令留到书签条这次消息处理
结束之后再执行，对话框不再嵌在书签条自己的消息处理里跑——对话框开着的时候书签条随时可能被
销毁，以前那是个悬空指针的隐患。

### 功能开关只留在托盘，并且对勾和实际状态一致

- 托盘里「书签」的对勾是缓存值，初值写死为「开」，启动时从不读配置、改完设置也不更新：配置里
  关着的书签，托盘照样打勾。现在托盘每次弹出都从当前配置现读四个功能的对勾，不再缓存。
- 设置页里的「启用书签」「启用边框」「启用置顶」去掉了，开关只在托盘一处。两处各记一份状态，
  就是上面那种对不上的来源。设置页的「确定」也不会再把对话框打开期间在托盘上切过的开关改回去
  （拖动设置对话框同样）。
- 「暂停所有 / 启用所有」的文字看的是书签、边框、置顶，点下去切的却是书签、边框、拖动：只开着
  置顶时显示「暂停所有」，点了反而把其余三个都打开。现在两边都是同一组四个功能。

### 左右两侧改到窗口内侧

侧边书签条以前挂在窗口外面，现在和横排一样贴在窗口内侧、往窗口里长，四个方向是同一套几何、
Top/Bottom 与 Left/Right 互为镜像。`placement=auto` 因此也不再需要看窗口外有没有地方：普通窗口
左侧、最大化底部，不会再随窗口移动在左右之间跳。默认仍是 `bottom`。
...
| 键 | 默认 | 含义 |
|---|---|---|
| `drawer.bottom_magnet_max_extent` | 72 | 横排：峰值宽度 |
| `drawer.bottom_magnet_max_thickness` | 36 | 横排：峰值高度 |
| `drawer.magnet_max_extent` | 56 | 侧边：峰值时伸进窗口的深度 |
| `drawer.magnet_max_thickness` | 52 | 侧边：峰值高度 |
| `drawer.magnet_radius` | 120 | 磁场半径 |
| `drawer.magnet_grace_ms` | 60 | 离开整条栏的宽限 |
| `preview.title_gap` | 7 | 书签到标题 |
| `preview.thumbnail_gap` | 10 | 标题到缩略图 |
| `preview.crossfade_ms` | 100 | 主标签切换的交叉淡入淡出 |

退役：`drawer.expanded_extent`、`drawer.bottom_expanded_extent`（磁性栏里没有「展开」）、
`drawer.attach_overlap`（书签条不再挂在窗口外）。配置文件里照常读写，不报错、不丢值，只是不再
生效；设置界面里由峰值取代。已有的 `settings.conf` 不会被改写，新增的键缺省时取上表的默认值。

### 验证

- 核心库新增两组纯函数：`MagneticDock`（磁场）和 `PreviewStack`（三段排布）。单元测试覆盖峰值、
  0.25px 步长扫过整条栏的连续性、间隙无死区、不动点、无反馈、远处不动、主标签滞回、四方向镜像
  与互不重叠。变异测试 12 + 8 个全部被抓住。
- 书签条窗口的几何（`LayoutEngine::DockSpecFor` / `ComputeOverlayBounds`）：鼠标在任何位置、磁场
  全开时，每个标签都完整落在窗口里（按 0.5px 扫一遍）。变异测试 13 个全部被抓住。
- `StackCheck.exe`（开发工具，不进安装包）离屏把四个方向、五种场景的三段式画成 PNG，逐个断言
  互不重叠、都在工作区里，用的是程序里同一份标题排版代码。竖排方向、省略号、空间不足时缩略图
  缩小，都是从这些图上看过的。
- 「像手指滑过钢琴键」是手感，只能在实机上看。

## v0.5.3

### 装了 WGestures 这类鼠标手势软件后，所有边框都消失

用户报告，二分到 v0.4.8 正常、v0.4.9 起不正常。

WGestures 常驻一个铺满全屏、置顶、完全透明的窗口画手势轨迹——眼睛看不见，系统却说它
「可见」。v0.4.9 起边框改成每屏一张画布，谁挡住谁由 WindowMark 自己用矩形算，这种窗口被
当成一块不透明的板子，屏幕上所有边框被整圈裁光。v0.4.8 是每个窗口一个边框窗口、遮挡交给
系统合成，所以不受影响。

改成**鼠标能穿过去的窗口不算遮挡物**。透明度从外面问不出来，命中测试却问得出来；而且两者
在现实里几乎总是一致的：一个铺满全屏、置顶、鼠标又穿不过去的窗口会让整个桌面点不动，只有
真锁屏、全屏程序会这样，它们本来就该挡住边框。

跨进程让鼠标穿透有两种做法，都要求 `WS_EX_LAYERED`：

- 再加 `WS_EX_TRANSPARENT`：样式位一看便知，不用做命中测试
- 靠 alpha 为 0 的像素：样式位上看不出来，只能在窗口里取五个点做命中测试（正中加四个象限
  的中心）。任何一处接得住鼠标就仍然算遮挡——录屏软件那种中间透明、四周一圈可见框的选区
  窗口，四个象限的点会打中那圈框

第二种会不会被命中测试穿过去，事先没有把握，是实测出来的。`tools\check-transparent-overlay.py`
自己造这两种窗口，数屏幕上边框颜色的像素：

| 覆盖层 | 修复前 | 修复后 |
|---|---|---|
| `WS_EX_LAYERED` + `WS_EX_TRANSPARENT` | 22,868 → **0** | **100%** 保住 |
| 只有 `WS_EX_LAYERED`，像素全透明 | 22,868 → **0** | **100%** 保住 |
| 对照组：同样全屏透明，但正中一块接得住鼠标 | 裁光 | 照样裁光 |

对照组有两个作用：证明修复没有矫枉过正，也证明 WindowMark 确实对测试窗口做出了反应——
否则「边框保住了」可能只是它根本没看见测试窗口。

两种情况照旧算遮挡：用户在 `tracking.treat_as_topmost_classes` 里点名的窗口（明说了要边框
给它让路）；被禁用的窗口（比如开着模态对话框的主窗口，命中测试本来就会跳过它，测出「穿过去」
不可信）。判定抽成核心库的 `PassesMouseThrough` 纯函数，7 个变异体全部被单元测试抓住。

`WindowMarkDiag.exe` 同步：穿透窗口不再被列为「挡住它的窗口」，并标注「鼠标可穿透，不算
遮挡物」。

没有退回 v0.4.8 那种每个窗口一个边框窗口的做法。那种做法把遮挡交给系统，却要自己不停地用
`SetWindowPos` 维护全局 z 序——任何进程都能改它、调整还会静默失败，v0.4.9-rc1 列出的层次错乱
和快切残缺都出在这里。现在这类问题归结为「哪些窗口不算遮挡物」，能用有依据的规则整类解决。
