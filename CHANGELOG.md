...
在全新安装的 Windows 10 上，`WindowMarkSetup.exe` 和 `WindowMark.exe` 双击直接报「找不到
VCRUNTIME140.dll / MSVCP140.dll / VCRUNTIME140_1.dll」。

CMake 默认动态链接 C/C++ 运行库（`/MD`），这三个 DLL 得由系统提供；全新系统、LTSC、IoT 版上
常常没有。用 dumpbin 核对过：0.4.1 到 0.5.2 每一个发布版的这三项依赖、导入的函数（84 / 11 /
1 个）和链接器版本都完全相同——所以缺运行库的机器上**哪个版本都起不来**，这件事和「0.4.8
能用、0.4.9 起不行」无关，那个是 SmartScreen 信誉（见 v0.5.2）。

改成静态链接（`CMAKE_MSVC_RUNTIME_LIBRARY=MultiThreaded`）。六个 exe 用 dumpbin 确认已经没有
`MSVCP140`、`VCRUNTIME140*`，连 UCRT 的 `api-ms-win-crt-*` 也没有了。代价是体积：
WindowMark.exe 537 KB → 761 KB。

0.5.2 及以前的版本仍然需要运行库，安装方法见 `WindowMark-依赖安装.md`。

### 书签盖在右键菜单上面

资源管理器里右键，弹出的菜单被书签条压住了一块。

书签条建成 topmost 窗口——为了绕开「不在前台的进程抬高窗口会被系统静默忽略」这条限制，
这是它能待在宿主窗口上面的前提。但 topmost 窗口之间谁在上面，看谁**最后一次被显示**：
右键时焦点在宿主和菜单之间晃一下，书签条随之隐藏、再显示，一显示就跳回 topmost 层的最顶上，
把刚弹出来的菜单盖住。宿主是前台窗口时 `SyncZOrder` 原来什么都不做（「保持置顶」）。

改成宿主在前台时把书签条挪到 topmost 层的**末尾**：比所有普通窗口高，比别的程序的 topmost
窗口（右键菜单、输入法候选框、任务栏、悬浮小窗）低。往下挪不算抬高，不会被拒绝——边框画布
靠同一个做法已经跑了几个版本。两处细节：

- 找锚点时跳过自家的窗口。边框画布也在往层末尾挪，而书签要压在边框线上面；把画布当锚点的话，
  两边会互相把对方当锚点，每次更新都换一次位置。
- 宿主自己是置顶窗口（被钉住的，或本来就 always-on-top）时不这么挪——层末尾在它下面，书签
  搭在窗口边上的那几像素会被宿主盖住。这种情况照旧贴在宿主正上方。

稳态下（已经在层末尾）不发任何 `SetWindowPos`，拖动宿主时每次几何更新只多一趟遍历。

顺带修正 `IsOwnDecoration`：它认的一直是 v0.4.9 之前每个窗口一个的 `WindowMark.WindowBorder`，
那种窗口早就没了，现在的边框画布 `WindowMark.Overlay` 反而不认。

## v0.5.2

### 双击没反应：新版本被 SmartScreen 静默拦下

网友报告：Win11 IoT 企业版 LTSC 上 0.5.1 画不出边框，以管理员身份运行也不行；Win10 LTSC 那台
「今天也没了」；一路往下试，只有 0.4.8 正常。开发机上换到另一台电脑后复现出了一模一样的
现象，还多一个线索：**0.4.8 能启动，启动后托盘里显示的版本却是 0.5.1**。

根因不在代码里：

- 从 GitHub 下载的 zip 带「来自网络」标记，解压出来的每个文件都继承了它（实测项目目录里
  466 个文件 464 个带着 `Zone.Identifier ZoneId=3`）。WindowMark 的 exe 没有数字签名，带着
  这个标记双击时要先过 SmartScreen。
- SmartScreen 按下载信誉决定要不要弹「无法识别的应用」。0.4.8 发布得早、信誉够了，不弹；
  之后的版本都得弹。
- 弹窗宿主 `CHXSmartScreen.exe` 在那台机器上**一启动就崩**：事件日志里 5 分钟内崩了 7 次，
  都崩在 `edgehtml.dll`，异常 `0x8000ffff`（系统更新装了一半，WinSxS 里暂存着 .9444、System32
  里还是 .8117，CBS 标着等待重启）。弹窗没出来，也就没有「仍要运行」，启动被静默放弃。
  LTSC / IoT 版精简掉了这类组件，是同一个结局。
- 「0.4.8 显示 0.5.1」：当时 0.5.1 已经在后台跑着。0.4.8 启动后单例检查发现已在运行，把
  消息转给它就退出了，托盘里那个一直是 0.5.1。

验证是直接的：同一个程序，本地编译出来的（没有网络标记）双击就起、全程不碰 SmartScreen；
对发布包执行「解除锁定」后，发布包里的 exe 也一样。

代码改不了这件事——被拦在我们任何一行代码运行之前。真正的解法是代码签名；在那之前，
README 的「快速开始」加上了「解压前先解除锁定」，并新增「双击没反应 / 看不到边框」一节。

### WindowMarkDiag.exe：出问题时双击运行，把报告贴回来

报告存到桌面、复制到剪贴板。「发现的问题」打头，下面依次是系统、显示器、WindowMark 进程和
边框画布、文件的网络标记、配置、边框规划回放、屏幕取样、WindowMark 自己的日志末尾。

凡是「WindowMark 会怎么判断」的部分，调用的都是 WindowMark 自己的代码（配置位置解析、窗口
资格判断、桌面快照、边框规划），不另写一份规则——另写的规则会漂移，然后自信地报出错误
答案，`WindowMarkInspect` 就吃过这个亏。

- **只读**：配置文件只在存在时才读（`LoadOrCreate` 在文件不存在时会创建它）。
- **隐私**：不收集窗口标题，路径里的用户目录换成 `%USERPROFILE%`，排除名单只报文件名。
- 配置的「exe 同目录」按**正在运行的那个** WindowMark.exe 算，不按诊断工具自己的目录。
- 第一版在远程桌面窗口最小化时误报「规划里有边框，屏幕上一个边框色都没取到」：截屏其实
  直接失败了，拿回来的是全零缓冲，取样必然零命中。现在先确认截屏成功、图里不止一种颜色，
  截不到就明说「没法检查」。

### 构建：换到代码页 936 的机器上编译不过

`/utf-8` 以前逐个 target 加，核心库 `windowmark_core` 漏了；原来的开发机开着系统级 UTF-8，
把它掩盖了。MSVC 按 GBK 读无 BOM 的 UTF-8，中文注释行尾的字节和换行拼成一个 GBK 字符，把
下一行代码吞进注释，报出「border 不是 Settings 的成员」这种和原因毫不相干的错。资源编译器
是同一个坑而 `/utf-8` 管不到它，`ClipKeeper.rc` 报 RC2133。两处都改成在 `project()` 之后全局
设置（`/utf-8` 和 rc 的 `/c65001`）。

PowerShell 脚本也是：Windows PowerShell 5.1 遇到无 BOM 的脚本按系统代码页读，中文字符串把
引号吃掉，`reinstall.ps1` 整个解析失败。**这一条影响用户**——发布包里带着的
`measure-shadow-inset.ps1` 在普通中文 Windows 上一样跑不起来。7 个含中文的 `.ps1` 全部加上
UTF-8 BOM，逐个用 5.1 的解析器确认 0 错误。

## v0.5.1

### 托盘图标在 explorer 重启后永久消失

现象是「WindowMark 启动不了」：托盘里没有图标，双击 exe 也毫无反应。

其实进程一直好好跑着。explorer 每次重启都会销毁所有托盘图标，然后广播一条
`TaskbarCreated` 消息让各程序重新加回去——WindowMark 从来没接过这条消息，图标于是再也
回不来。explorer 重启比想象中常见：改显示配置、ToDesk / 远程桌面接入、它自己崩一次都会。
实测这次就是：WindowMark 15:15 启动，explorer 18:46 重启。

双击 exe 没反应是第二重：单例检查发现已在运行，就给运行中的实例发个消息、自己退出；
而那个消息的处理是 `ShowAlreadyRunningHint()`，它用 `NIM_MODIFY` 往图标上挂个气泡——
图标都没了，`NIM_MODIFY` 直接失败，而且是**静默**失败。于是用户最可能采取的动作
（双击 exe）恰好是唯一走到这条路的时候，却什么都不发生。

现在接上 `TaskbarCreated` 重新加图标；`ShowAlreadyRunningHint` 也不再假设图标存在，
`NIM_MODIFY` 失败就先加回来再挂气泡。

### 锁屏之后可能永远停在挂起

只认 `WTS_SESSION_UNLOCK` 一个恢复事件。机器锁过一次，之后走 ToDesk 之类的远程接入，
到达的是 `WTS_CONSOLE_CONNECT` / `WTS_REMOTE_CONNECT`，`UNLOCK` 未必来——于是永远停在
挂起：进程跑着、消息循环正常、一条边框也不画。

`overlays_.Sync()` 在挂起判断后面，所以画布尺寸也跟着冻住。实测抓到的现场：屏幕已经是
1920x1080，画布还停在 1536x864。

改成**只有 LOCK 挂起，其余每一个会话事件都恢复**。写成「其余一律恢复」而不是逐个列出
恢复事件：新的事件类型只会把它推向恢复，不会把它锁死在挂起里。多画几帧没人看的东西是
小事，永远不画是大事。恢复时走 `Redraw()`，里面的 `overlays_.Sync()` 会按当前显示器配置
重建画布，挂起期间的插拔和分辨率变化一并补上。

`tools\check-session-resume.py` 盯诊断日志里的状态机验证这件事，不盯屏幕像素——锁屏界面
或任何全屏窗口盖在上面时边框本来就该一个不画，那时「屏幕上没有边框」根本区分不出挂起和
被遮挡。把恢复分支变异回「只认 UNLOCK」，6 个恢复事件里 5 个立刻失败。
...
### `corner_width_extra` 默认 3 改成 1

加宽方向从「往窗口内」变成「往窗口外」之后，这个数的含义就跟着变了：以前它决定往窗口里
吃多少（3 → 吃 4px），现在决定往窗口外伸多远（`reach + extra`）。原来那个 3 是按「往内长」
在屏幕上逐档试出来的，换了方向不再是那个意思，所以重新定。

默认这套现在是 **3px 在外、1px 在内、整条线 4px**。设 0 就是严格的 `border.width`
（2 外 1 内）。

升级不会改写已有的 `settings.conf`，新默认值只对全新安装生效——想用新值得自己把
`border.corner_width_extra` 改掉，或者 `rebuild_and_install.bat -Fresh`。

## v0.5.0

### 窗口拖动

按住修饰键（默认右 Alt），**在窗口内的任意位置**按下鼠标就能拖它——不用够到标题栏，也不用
瞄准四条边。

主要解决一件事：**窗口跑到屏幕外面，只剩一个角还露着**。这时候标题栏和拖动区都在屏幕之外，
鼠标根本点不到，窗口就再也拉不回来了。

- **左键拖 = 整窗移动**，窗口内任意一点都行
- **左键单击 = 居中**，位移 4px 以内算单击，窗口回到光标所在那块屏的正中
- **右键拖 = 缩放**，按下的位置落在九宫格哪一格决定拉动哪几条边

默认关闭。托盘 →「窗口拖动」→「启用」。触发键六个预设可多选（左右 Ctrl / Alt / Win），
默认只勾右 Alt——左 Alt + 拖动被 Photoshop、IDE、Blender 占着，装在左 Alt 上会吞掉那些操作。

单击居中不占任何额外按键：那次按下本来就被拖动吞掉了，应用永远收不到，不接上就是白白浪费。

#### 拖不动，窗口只晃一下

`WM_MOUSEMOVE` 和按下/松开一样被吞掉了。`WH_MOUSE_LL` 返回非零 = 事件不再往下走，对移动来说
就是**光标不动**。真实鼠标报的是相对位移，系统拿「当前光标位置 + 这一跳的位移」算
`MSLLHOOKSTRUCT::pt`——光标被钉死，`pt` 就永远只在按下点附近晃一个鼠标 tick 的距离，永不累加。

这个 bug 躲过了原来那份合成测试，因为它注入的是**绝对**坐标：`pt` 不依赖当前光标位置，照样
累加，测试完全绿。把注入改成**相对**位移后，同一份测试立刻抓住了它——变异回原来的写法，报出
「光标位移 (0,0)、窗口只挪了 (-116,-117)」，和手上的现象一模一样。

判据也跟着改了：比「窗口位移 == 光标**真实**位移」，不比「== 注入的位移」（指针加速会把后者
带偏），并且单独断言光标确实动了——少了这一条，「窗口没动 + 光标也没动」会当成两者一致蒙混
过关，而那恰恰就是这个 bug 的样子。

#### 鼠标底下不能拖时，往 z 序下面找

系统会留下**看不见却铺满整块屏**的 topmost 窗口：锁屏的 `Windows.UI.Core.CoreWindow`
（LockApp.exe）和 `LockScreenInputOcclusionFrame`，解锁之后有时并不消失，`IsWindowVisible`
仍然是 1。抓现场时本机两块屏各挂着一个。

`TargetWindowAt` 原来只问一次 `WindowFromPoint`，拿到不能拖的就返回空，于是整个手势在那块屏上
彻底失效，而屏幕上什么异常都看不出来——最难查的那种。现在继续往 z 序下面找第一个能拖、且盖住
这个点的窗口；只在按下时走一次，不进移动的热路径。

### 性能

空闲路径只有两行：先看在不在拖动，再看修饰键按没按（`GetAsyncKeyState`，几十纳秒），不查窗口、
不算几何、不碰配置。装与不装钩子的差值落在噪声里。

键盘钩子**只在配置含 Win 键时才装**——单独按一下 Win 再松开会弹出开始菜单，要压掉被手势用掉的
那一次抬起只能靠 `WH_KEYBOARD_LL`；它比鼠标钩子更敏感，不该让所有人默认承担。

## v0.4.10

### 重装会把开机自启动弄丢

现象是重启电脑之后 WindowMark 没起来。查下去两边对不上：`Run` 键里没有启动命令了，而
`StartupApproved\Run` 里那条「已批准」还留着。

两段代码合起来造成的：卸载器关掉自启动（真卸载之后不该开机启动一个已经不存在的程序，
这是对的），而安装器读「当前状态」来决定装完要不要开——`reinstall.ps1` 是先卸载再安装，
安装器读到的正是卸载刚清空的那个状态。每升级一次就关掉一次。

升级脚本才是知道「我在升级」的那个角色，改由它把卸载前的状态记下来，装完用
`/StartWithWindows` 传回去（安装器本来就支持这个开关）。

顺带修了关闭路径的一个残缺：只删 `Run` 键的值，不删批准记录。留下的正是代码注释自己
说要避免的状态——批准记录指向一个不存在的启动项。查「为什么开机没启动」的人会先看到
那个 `02` 以为一切正常，而真正缺的是 `Run` 键里的命令。

### 边框不再压在任务栏上

窗口贴着工作区底边（半屏吸附，或手动拖到贴边）时，边框往外那两像素正好落在任务栏那条
上。实测：窗口内五像素命中 12 个采样点，窗口外命中 6 个。

跨屏那种越界不用管——画布只覆盖本监视器，越界的部分根本没地方落笔。压任务栏挡不住，
因为画布覆盖的是整块监视器（含任务栏那一条）。

`ClampBorderToScreen` 是 rc1 时代为独立边框窗口写的，这次接进 overlay 模型。接法上有个
坑：**不能把夹取后的矩形直接当成边框环的外矩形**——圆角的路径以它为基准算半径和位置，
改了弧就错位。外矩形保持不动，只把**可见范围**限制在夹取后的矩形里，弧还是原来那条，
只是压出去的那截不画。为此加了 `ClipToBounds`（和 `ClipSegments` 正好相反：那个减去
挡住的，这个只留框里的），带五组单测。

修完实测：窗口外从 6 个采样点降到 0，窗口内仍是 12——边框本身完好。
...
每块显示器一个透明的 topmost 画布，所有边框都画在上面。「边框该排第几」这个要和整个
系统抢一条全局 z 序链表的时序问题，就此变成「哪几段该画」的纯几何问题：非激活窗口的
边框环减去排在它上面的窗口，剩下的才画；激活窗口的边框完整画。

rc1 里那一整套 z 序操作——精确锚定、被拒后逐级退让、推不动就重建、守望轮询——全部
删掉。性能不降反升：脏矩形提交实测 0.91ms 中位数，rc1 的实现是 3.00ms。

### 圆角

直角走原来的像素填充，一个像素都没动。圆角自己逐像素栅格化——圆角矩形的有符号距离有
闭式解，一个像素算一次就知道它离环有多远。（中途用过 Direct2D，因为太慢换掉了，见下面
「拖动跟手」一节。）半径跟随 DPI 缩放——Windows 11 的 8 是 **DIP** 不是物理像素，
125% 下该画 10px。

设置里新增两项，只在圆角模式下可编辑：

- **圆角加宽**（`border.corner_width_extra`，默认 3）：在线宽上多加多少。弧是斜的，
  抗锯齿在两侧各留约 1px 渐变，实心部分因此比名义线宽窄（实测 `border.width=4` 时，
  名义 4 只剩 2px 实心）。多出来的宽度全长在窗口内侧，外沿不动。
- **圆角内移**（`border.corner_inset`，默认 0）：整圈往窗口中心挪多少，负数往外。

两个数都没法算准——抗锯齿吃掉多少取决于弧的曲率和它落在像素格的哪个位置——所以做成
了设置项，屏幕上试比公式可靠。

### 点一下桌面，所有边框消失

「其余窗口一律再减去前台矩形」这道保险，前提是「前台一定在最前，它盖住的地方不该有
别人的边框」。桌面窗口正好相反：它永远在 z 序最底、谁都不挡，而它的矩形是**整个虚拟
桌面**（实测 `Progman`，`(-1920, 0, 2560, 1440)`）。用户点一下桌面空白处它就成了前台，
那一减就是全屏，所有边框一次裁光。

现在桌面当前台时跳过这道保险。判据用 `GetShellWindow()`——系统给出的答案，不是猜类名；
`WorkerW` 另算：装了动态壁纸时桌面图标那层会变成它，它不是 shell window，但同样铺满
整个桌面、同样谁都不挡。

### 快速切换窗口后，别人的边框盖在最上面那个窗口上

来回快切两个窗口，停在哪个上面，别的窗口边框就整条画在它上面。

根因在快照的遍历方式：`GetTopWindow` + `GW_HWNDNEXT` 走的是**活的** z 序链表。走到
一半有窗口被提到链表前面，那个窗口就再也遇不上了——快速切换时这正在发生，实测切七八
次就能让前台窗口从快照里整个消失（前一帧 43 个窗口有它，下一帧 42 个就没了）。

漏掉前台是致命的：`PlanBorders` 的两道保险——「排在它上面的算遮挡」和「其余窗口一律
再减去前台矩形」——都要在快照里按 hwnd 找到它。找不到，两道全失效。

现在遍历完会检查前台在不在，不在就补进去。补的位置有依据：前台窗口排在所有非 topmost
窗口之前，这是「前台」的定义。修完实测连切 41 次没再复现。

（这个竞态一直存在，只是从前每帧都整取快照，下一帧就自己好了；v0.4.9 加了快照缓存
之后，没有新事件就一直卡在漏掉的那一帧上。）

### 接缝处再往里咬一像素

遮挡范围和被挡窗口正好齐平时，边框的最后一像素和它的第一像素挨着但不相交，两边各带
半像素的抗锯齿过渡，看着就是一条一像素宽的缝。遮挡矩形统一往里咬一像素之后，边框钻到
对方边缘底下，接缝消失。

只作用于「它挡住别人」这一面。给它自己画边框时仍用原矩形——那个要贴着它看得见的边缘，
跟着咬进去就该细一圈了。

### 边框在小窗旁边断开的口子比小窗还宽

有些窗口把阴影画在自己的窗口矩形里，矩形比看得见的部分大一圈——实测 snipaste 的贴图窗
四边各 14px、下边 32px。遮挡计算按矩形算，别人的边框就在它两侧各多断 14px，看着像边框
断了而不是被它挡住。

`tracking.shadow_insets` 本来就是修正这个的，但它此前只作用于「给这个窗口画边框」；
现在遮挡计算也用同一份值（见下一节）。配上之后同一条边实测：

| | 缺口 | 相对那个窗口的可见边缘 |
|---|---|---|
| 配置前 | 743..899 | 两端各多出 14px |
| 配置后 | 756..886 | 差 1px |

值不用手试：`measure_shadow_auto.bat`，鼠标停在那个窗口上五秒，它自己量出来打印成一行。
量法是沿垂直于边的方向扫像素——阴影是渐变的，实体边缘是突变，取梯度最大的位置。弹出
面板也能量：鼠标停在上面它不会关。

### 边框不对劲时，自己就能改

边框出问题的形态就那么几种，用户自己能对上号的也就三句话。设置里「按类名调整」一组
三个框，一一对应（类名用 `WindowMarkInspect.exe` 查）：

| 症状 | 填哪个 | 配置键 |
|---|---|---|
| 这个窗口不该有边框却有 | 不画边框 | `tracking.exclude_classes` |
| 该有边框却没有 | 强制画 | `tracking.force_include_classes` |
| 它浮在上面，边框却盖了过去 | 视为置顶 | `tracking.treat_as_topmost_classes` |

「强制画」跳过所有「这算不算一个窗口」的资格判据。那套判据是用来挡掉工具窗口、悬浮
小卡片、对话框的，判据一多误伤就一定存在，而误伤了本来没有别的办法绕过。排除名单仍然
优先——两边都写了就当没写强制的，「不要画」比「一定要画」保险。

「视为置顶」让别人的边框给它让路。前台窗口的边框本来只被两类东西裁：真正的 topmost
窗口，和它自己的 owned 对话框。可有些窗口既不是 topmost 也不属于前台，却实实在在浮在
上面（各家自绘的浮动面板、输入法候选框、悬浮工具条），边框就会从它们身上穿过去。

### 阴影内缩现在也管遮挡

`tracking.shadow_insets` 修正的是「窗口矩形比它看得见的部分大一圈」，此前只作用于给
这个窗口画边框，它**挡住别人**时用的还是没修正的大矩形。结果是别人的边框在它旁边断开
的那一截比它本身更宽——看着像边框断了，而不是被它挡住。

现在两处用同一份值。量法没变：`measure_shadow_inset.bat`，鼠标悬停五秒，把打印的那行
填进配置。

### 对话框被边框线切开：事件层和遮挡层的口径不一致

MobaXterm 的设置对话框上横着几条别的窗口的边框线，拖动对话框也不消失。

根因是同一个对象在三层里用了三套判据：

| 层 | 判据 | 对这个对话框的结果 |
|---|---|---|
| 跟踪层 `IsEligibleTopLevelWindow` | owner 是个有尺寸的真窗口就排除 | 不给它画边框——对的，它是主窗口的对话框 |
| **事件层 `IsTopLevel`** | **只看 `WS_CHILD`** | **它的 SHOW / HIDE / LOCATIONCHANGE 全被丢掉** |
| 遮挡层 `CaptureDesktop` | 可见且在桌面的顶级链上 | 收它，参与遮挡计算 |

这个对话框是 Delphi 的 `TFormParams`：**既带 `WS_CHILD`、又直接挂在桌面下**，出现在顶级
窗口链上、盖得住别的窗口。于是拖动它的时候一个事件都产生不了——诊断日志文件压根没
生成，这是最硬的证据——而遮挡计算那边还记着它的旧位置，别人的边框就按旧位置裁，留在
了它身上。

事件层的判据改成 `GetAncestor(hwnd, GA_ROOT) == hwnd`。给不给它画边框是一回事，它动
没动、挡没挡住别人是另一回事，后者必须和遮挡层同一个口径。

### 拖动跟手：CPU 从 98% 降到 20%

拖动窗口时边框落后十几个像素，还有近一半时间整条画不出来。量下来是一个核被占满了：
边框每秒重画 30 次，每次 34ms。

分三处修，每一处都是先量再改：

**一、圆角自己栅格化，不再用 Direct2D。** 圆角段原本是「画整个环，再裁到这一段」，
一个窗口每帧有六段可见，就把周长近四千像素的整环栅格化了六遍，而且 `BindDC` 的
render target 走的是 CPU。实测每段 2.15ms，一秒 750ms。圆角矩形的有符号距离有闭式解，
一个像素算一次就知道它离环有多远，代价是一次 `sqrt`；再从裁剪区里挖掉中间那一大块
空洞，逐像素的循环就只剩环那一圈带子，一个窗口约三万像素。**每段 2.15ms 降到
0.044ms。** 顺带把抗锯齿的过渡带宽度收回自己手里，不再由 D2D 说了算。

**二、脏区只算变了的段。** 提交给 `UpdateLayeredWindowIndirect` 的脏矩形原本取所有
边框段的包围盒——拖一个窗口时别人的边框一个像素都没动，却每帧都要重贴大半个屏幕
（实测平均每帧 2.41 百万像素，屏幕总共 3.69 百万）。改成和上一帧的段列表比出增删，
只有变了的才进脏区。**每帧 2.41 降到 0.68 百万像素。**

擦除和重画的口径必须一致：擦掉脏区之后，要把**所有**和它相交的段重画一遍，而不是
只画变了的那些——擦除会连带抹掉压在同一块地方的其他段。

**三、几何事件不再整取桌面快照。** 拖动时每秒上百个几何事件，每个都遍历全部顶级窗口、
对每个可见窗口做两次跨进程 DWM 查询，实测每次 2.4ms。而这期间动的只有一个窗口。改成
缓存快照、几何事件只刷新那一条；除几何以外的任何事件都走 `Apply`，那里让整份快照作废。
**每秒 260ms 降到 28ms。**

结果：CPU 占用 98% → 20%，边框重画频率 30 次/秒 → 125 次/秒（跟得上鼠标的报告率），
实测滞后从中位 12px、p90 17px 降到 0px，「整条边框没画出来」的采样从 47% 降到 0%。

自绘这条路上踩了两个几何坑，都补了测试：

- **距离公式的短路少带了一个分量。** 写成「x 落在直边范围内就返回 0」，把 y 方向的
  分量一起丢了，于是整条上下边的距离算成 `-radius`，覆盖率为负，一个像素都不画，而
  左右边（走另一支）完好——现象是边框只画了一半。
- **挖洞挖到了环上。** 挖的是直角矩形，而环的内沿在角上是圆弧，直角的角顶进了环带。
  半径 11、半线宽 3 时洞的角点离弧心 9.9，环带覆盖 [8, 14]，正好在正中间。现象是圆角
  内侧被切出一条直的台阶，紧挨着的那一行相对凸出一个小直角。

两个都是埋在三层循环里的纯算式，抽成 `RoundedRectDistance` 和 `RingHoleInset` 之后就
能单测了。

新增两个工具：`tools/bench-drag-lag.py` 量边框落后窗口多少像素，
`tools/bench-simulated-drag.py` 用 `SetWindowPos` 驱动一次可重复的拖动并读回耗时统计
（诊断开关是 `%LOCALAPPDATA%\WindowMark\diag.on`，`Redraw` 的分段耗时写进 `diag.log`）。

### 边框默认值换了一套

装上就有边框（`border.enabled` 从 `false` 改成 `true`），线宽 3、偏移 -1、圆角自定义
12、圆角加宽 3、圆角内移 0。这套值是在屏幕上一档一档试出来的，不是算出来的。

圆角默认走「自定义」而不是「跟随系统」：跟随系统拿到的是窗口**自己**那个角的半径，
而边框套在它外面、走的是一条更宽的弧，照抄过来画出来比窗口方一圈。自定义的代价是
它是物理像素、不跟 DPI 缩放——14 是 125% 下的值，别的缩放比例要自己调。

已有的 `settings.conf` 不受影响，只有新装和重置才用这套值。

### 设置对话框：字段联动

「自定义圆角」只在圆角模式选到「自定义」时可填，「圆角加宽」「圆角内移」只在不是
「直角」时可填，其余时候灰掉。依赖关系按**标签**查找而不是按下标，字段表随便增删排序
都不会悄悄失效。

## v0.4.9-rc1

预发布。边框的 z 序做了大量修复，**但没有根治**——已知问题见本节末尾。正式版会换成
另一套模型，见 `docs/superpowers/specs/2026-09-05-边框Overlay重构-design.md`。

### 最大化的窗口不再画边框

最大化窗口贴着工作区边缘，外面没有画边框的那 3px；夹回屏幕内之后边框和窗口边界完全
重合，trace 实测四条边**一个像素都露不出来**。而它正是 v0.4.5–v0.4.8 那三轮「溢出到
任务栏」「跨屏跑到另一块屏」的源头——为一个看不见的东西修了三个版本。

置顶窗口例外：置顶是显式操作，边框是它生效的唯一视觉反馈。

### 边框 z 序：三个根因

**一、`SWP_NOSENDCHANGING` 不能用于 z 序调整。** 带着它设 `HWND_TOPMOST` 会「假成功」：
返回 TRUE、`GetLastError()` 为 0、topmost 位和 z 序立刻都变了（实测 z 从 106 跳到 12），
但两百毫秒内又退回原处。它跳过 `WM_WINDOWPOSCHANGING`，而那是 z 序变更提交路径的一部分。
这个 flag 对**移动**是合理优化（`Reposition` 里保留），对**改 z 序**是错的。

**二、精确锚定会被 UIPI 拒绝。** 让边框贴在目标正上方需要指定一个锚点窗口，锚点属于
完整性更高的进程时直接被拒（任务管理器实测 25 秒内 141 次），而且**不区分方向**——往它
下面插同样不行。

**三、判据不能只看自己维护的 active 标记。** 它和 `GetForegroundWindow()` 在焦点切换
途中会分家，那时提 topmost 会在别的窗口上留下一条与谁都不搭界的孤立线。

现在的规则：前台窗口用一个**创建时就带 `WS_EX_TOPMOST`** 的专用边框窗口（只移动、只
上色、层内换位，永不执行「从普通层提进 topmost 层」——那正是会卡死的操作）；其余窗口
的边框紧贴在各自目标上方，被拒则依次退到目标下方、再下一格、沉底。

配套：前台切换钩子（Windows 只把 `EVENT_SYSTEM_FOREGROUND` 发给**新的**前台窗口，刚
失去前台的那个收不到通知）；z 序推不动的边框窗口自动重建。

### 性能：拖动跟随与焦点切换

三处都不是计算量的问题，是顺序和节流：

- `MoveTo` 的 15ms 节流与 60Hz 的 16.7ms 打拍，每隔几帧丢一帧（拖动残影）。`Reposition`
  本来就有「位置没变就跳过」的短路，这个节流是纯损失。
- z 序排在重绘后面，而视觉上「边框出现」等的正是 z 序；重绘要把 5.4MB 位图交给 DWM。
- 书签条的 `ApplyModels()` 排在边框的 `ApplyBorders()` 前面，四个调用点都是。

| 指标 | 改前 | 改后 |
|---|---|---|
| 拖动跟随·中位数 | 6.1 ms | **3.0 ms** |
| 拖动跟随·p99 | 45.3 ms | **15.7 ms** |
| 拖动跟随·一帧内 | 95% | **100%** |
| 切换后边框出现·中位数 | 28 ms | **5 ms** |
| 切换后边框出现·最慢 | 141 ms | **11 ms** |

残影来自**尾部**而不是中位数：去掉节流对中位数几乎没有影响（3.6→3.0ms），而 p99 从
27.2ms 掉到 15.7ms。

### 修复：MobaXterm 等 Delphi/VCL 程序完全没有边框

排除附属窗口的判据是 `GetWindow(hwnd, GW_OWNER) != nullptr`，而 **Delphi/VCL 把主窗体挂
在一个隐藏的 `TApplication` 窗口下面**（实测 0x0 尺寸 + `WS_EX_TOOLWINDOW` + 可见），
于是所有 VCL 程序的主窗口都被当成附属窗口滤掉了。

改为看 **owner 自己像不像个窗口**：有实际尺寸、且不是 TOOLWINDOW，才算真的拥有者。
验证时**同时验了反面**——真对话框仍然被正确排除。

### 加固：内缩量缓存键补上 DPI

缓存键原本只有「尺寸 + 最大化状态」，两块屏缩放不同时，窗口跨过去而物理尺寸恰好没变
的话会一直用旧屏的内缩量。**这条是照机制补的防线，未经复现**——本机两块屏同为 125%。

关于反馈的「从小屏移到大屏，底部画线不是高了就是低了」：在真实双屏上测了四组场景，
都没能复现。

### 已知问题（rc1 未解决）

- **非激活窗口的边框层次经常不对**：会被与目标重叠的其他窗口盖住，或夹在错误的位置。
- **来回切换多个重叠窗口时，边框仍会出现短暂的不完整或闪烁。**

两者同源：现在用**全局共享、任何进程都能改、调整还会静默失败**的 z 序，去表达十几组
局部的归属关系。每个边框的 `SetWindowPos` 都改动同一条全局链表，后一个的插入打乱前一个
刚排好的位置，窗口越多越乱。这不是补丁能修好的，正式版会改成「Z 序只读不写 + 遮挡裁剪」
的单 Overlay 模型。
...
| 工具 | 用途 |
|---|---|
| `tools/check-border-band.py` | 边框落在哪一层 |
| `tools/check-floating-borders.py` | 查孤立浮线 |
| `tools/check-maximized.py` | 最大化窗口有没有残留边框 |
| `tools/bench-border-latency.py` | 自建测试窗口量拖动跟随延迟 |
| `tools/bench-switch.py` | 被动量切换窗口后边框出现的延迟 |
| `tools/trace-borders.py` | 逐窗口打印矩形、z 序、四条边的像素命中 |
| `tools/check-tools.py` | 静态扫出工具脚本里「用了但没定义」的名字 |

最后一个是因为 Python 的 `NameError` 只在执行到那一行才报，而这些工具里最要紧的代码
（事件回调、后台线程）恰恰只在特定时机才跑。

## v0.4.8

### 修复：边框溢出（v0.4.5 那次没修干净）

使用者反馈：全屏时仍然溢出；普通窗口偶现右边和下面溢出，**拉伸一下就恢复**；Excel、
360 浏览器会出现，WPS 不会。

从录屏抽帧量到边框右边界约 **1928**，而屏幕边界 1920 —— 溢出约 8px，正是本机实测的还原态
内缩量。顺这条线查出**三处独立根因**：

**一、底边夹错了参照物。** 最大化窗口的 DWM 边界等于**工作区**（1020），离监视器底（1080）
差着整条任务栏，判据 `0 <= gap <= reach` 永远不成立，底边从来不夹，边框画到 1023。
v0.4.5 的说明里写「底边落在任务栏区域，被任务栏遮住，无害」——**这是错的**：边框窗口带
`WS_EX_TOPMOST`，不会被任务栏遮住，那 3px 就画在任务栏上面。使用者截图里那条穿过任务栏的
红线就是它。

改为**工作区优先、监视器兜底**：最大化和半屏吸附贴的是工作区；用户手动拖到盖住任务栏的
窗口贴的是监视器，两档都要有。

**二、动画期间标定出的内缩量被当成可信值缓存。** 最大化和还原都有动画，`GetWindowRect`
立刻返回终值而 DWM 边界还在路上，那一刻算出的内缩量**是错的但非零**，于是被
`if (!zero) return true` 放行；缓存只在尺寸变化时重标定，最大化后尺寸不再变，**就一直错到
用户拉伸窗口为止**。这解释了「偶现」「拉一下就好」，也解释了为什么不同应用表现不同——
窗口动画时长和 DWM 更新时机各不相同。

加了一条合理性检查：内缩量实测都是个位数（还原 `8,0,8,8`，最大化 `9,9,9,9`），超过 32 只
可能是读到了上一个状态的边界，不予缓存。

**三、缓存键漏了最大化状态。** 把窗口手动拉到正好等于工作区大小再最大化，`GetWindowRect`
尺寸不变而内缩量从 `8,0,8,8` 变成 `9,9,9,9`，只比尺寸会认为不必重标定。

**附带**：把 `MonitorFromRect` 换成 `MonitorFromWindow`。原来的理由（「不依赖 HWND 是否
有效」）站不住——`frame` 本身就是从这个 HWND 算出来的、还隔着一层缓存，frame 一偏就会连屏
都选错，夹到隔壁屏的边界上。修复不该依赖它要修的那个可疑输入。

验证：反复最大化/还原 12 轮，边框与工作区逐像素相等；全屏幕 9 个窗口用独立实现交叉比对
全部一致，其中包含 Excel 最大化。**双屏场景本机复现不了，需要实机确认。**

## v0.4.7

### 补上 LICENSE（MIT）

此前仓库**没有任何许可证文件**。已经公开在 GitHub 上并发了多个 release，但在法律上默认是
「保留所有权利」——严格讲别人连合法 fork 都不行，更不用说使用或修改。

现在是 MIT：可以自由使用、修改、闭源、商用、再分发，只需保留版权声明，不提供任何担保。

代码全部原创，没有引入任何 copyleft 许可证的第三方代码。窗口边框的**想法**来自
tacky-borders（Rust），这里是独立的原生实现而不是移植，所以不受它的许可证约束。

## v0.4.6

### 新增：剪贴板守护（ClipKeeper）

针对 **ToDesk 等远程会话中截图后 `Ctrl+V` 失效**——`Win+V` 历史里看得到图片，但各处粘贴
都变灰。ClipKeeper 抢占剪贴板监听链的链头，在内容刚写入时先存一份，随后若被清空则自动恢复。

**它是独立进程，不是 WindowMark 的一部分**，两条理由都是硬的：

1. 它用 `SetClipboardViewer` 抢**链头**，而 Windows 只通知链头、由链头往下传——谁最后注册
   谁在最前面。WindowMark 开机自启，必然早于 ToDesk；排在后面时轮到自己剪贴板已被清空，
   没有东西可存。独立进程才能在**连上 ToDesk 之后**再启动。
2. 它每次剪贴板变化都要 `SendMessageTimeout` 往下游传（超时 250ms）。放进 WindowMark 的
   主消息循环，下游一卡边框刷新就跟着卡。

用法：托盘菜单 →「剪贴板守护」。这一项开关的是**面板**——点一下没跑就启动，面板开着就收
起到它自己的托盘（**进程继续守护**），在托盘里就叫回来。停止守护在 ClipKeeper 自己的面板里做。

**不随 Windows 启动，也不随 WindowMark 启动**，这是唯一能保证它排在 ToDesk 前面的做法。
它面板上的「开机启动」勾选框保留但不建议开——开了就回到「先于 ToDesk 启动」的状态。

安装器一并安装，卸载器先停掉它再删目录（运行中的 exe 删不掉，会让整个删除失败）。
它的版本号独立于 WindowMark，保持 `0.1.0`。

### 改进：托盘菜单一眼看出哪些功能开着

「书签」「窗口边框」「窗口置顶」左边现在按各自的启用状态显示对勾，右击一次就看得到，
不必逐个展开子菜单。切换仍在子菜单第一项——Win32 里带子菜单的项点击即展开子菜单，
系统不给它命令 ID，没办法兼作开关（`MF_OWNERDRAW` 也绕不过去，它只让你自绘，不让你细分
点击区域）。「剪贴板守护」没有子菜单，是顶层唯一能真正点击切换的。

同时**去掉了顶上灰色的「WindowMark」标题行**。实测菜单宽度只由最宽的标签决定，而那十个
拉丁字符比任何中文标签都宽：去掉后菜单从 212px 收到 187px。讽刺的是这个标题当初正是为
减宽才加的（把「关于 WindowMark...」拆成标题 + 「关于」），现在自己成了瓶颈。

顺带纠正一个直觉：`MF_CHECKED` 完全不参与菜单宽度计算，带勾不带勾都是 212px。
...
反馈原文：双屏显示时 Office 三件套最大化，边框会显示到另外一个屏幕去，溢出位移很大。

边框画在窗口可见边界外 `reach` 像素，`reach = border.width + border.offset`，默认
`4 + (-1) = 3`。实测（单屏 1920×1080，工作区 1920×1020）：

| 状态 | `GetWindowRect` | DWM 扩展边界 |
|---|---|---|
| 还原 | (691,162)-(1854,892) | (699,162)-(1846,884) |
| **最大化** | (-9,-9)-(1929,1029) | **(0,0)-(1920,1020)** |

关键在最大化那一行：**DWM 扩展边界正好等于工作区**。于是边框矩形是
**(-3,-3)-(1923,1023)**，右边那 3px 超出屏幕。单屏被裁掉看不见，双屏则越过分界线
落到隔壁屏上——就是反馈截图里那条竖线。同一根因也影响半屏吸附（Win+←）。

**没有采纳反馈里「最大化时不显示边框」的建议**：双屏各有一个最大化窗口时，活动/非活动
的配色正是用来区分焦点在哪边的；最大化窗口被置顶后也会失去唯一的视觉反馈；而且半屏
吸附的越界依旧存在。用一个功能缺失去掩盖坐标越界，是把问题藏起来。

改为逐边夹回窗口所在监视器，判据 `0 <= gap <= reach`。**下界不是防御性代码**：手动
横跨两块屏的窗口，边缘落在本监视器外面（gap 为负），没有下界那条边会被误裁。

实测结果：

| 状态 | 外扩量 | 结果 |
|---|---|---|
| 还原居中 | 3,3,3,3 | 与改动前一致，无回归 |
| 最大化 | 0,0,0,3 | 边框右边 1920（改动前 1923） |
| 贴左边缘 | 0,3,3,3 | 左边收进去，右边正常外扩 |

底边不夹是有意的：最大化时它落在任务栏区域，会被任务栏盖住。

### 新增：配置文件可以放在别处（便携 / 自定义路径）

反馈原文：C 盘装了还原工具，重启后配置就被还原了。

查找顺序改为三层：

```
1. exe 同目录的 settings.conf              存在即用（便携，跟着程序走）
2. HKCU\Software\WindowMark\ConfigPath     托盘「配置文件...」里指定
3. %LOCALAPPDATA%\WindowMark\settings.conf 默认，与以前一致
```

**便携排在显式指定之前**：注册表跟着机器走，exe 旁边的 conf 跟着程序走。U 盘插到别人
电脑上，不该去读那台机器注册表里指向的本地路径。代价是可能反直觉，所以对话框顶部
始终显示当前实际生效的那一份。

托盘菜单新增「配置文件...」，三个选项正好对应上面三层。切换时会把现有配置搬到新位置；
从便携切走时，exe 旁那份改名为 `settings.conf.disabled`（不删除——那是用户的数据），
否则下次启动第 1 层又会把它抢回去。指定到写不进去的位置会当场拒绝，不会看起来成功
却存不进去。

**这个功能解决不了还原工具本身。** 见 README「配置文件位置」一节：任何留在 C 盘的
东西都会被还原，包括注册表和程序目录，唯一可靠的做法是把整个 WindowMark 目录搬到
非 C 盘运行。
...
窗口的边框会一直偏出去 8px，除非改变窗口大小才归位。实测 YeImageViewer：边框在
DWM 边界外 **11,3,11,11**，而应当是 3,3,3,3；把宽度改 1px，立刻变成 3,3,3,3。

多出来的正是 `GetWindowRect` 与 DWM 边界的差值 **8,0,8,8**（上边差 0，所以上边看着是对的）。
`FrameFor()` 把这个差值按「窗口 + 尺寸」缓存，只在尺寸变化时重新标定。如果标定发生在
DWM 还没就绪的时刻——窗口刚创建，或刚从最小化还原——差值被记成 0，之后尺寸不变就一直
按原始窗口矩形画。

改成：**不可信的标定不入缓存**，下一次几何更新自动重来。不可信有三种：

- DWM 没答上（`ExtendedFrame` 原本会静默退回 `GetWindowRect`，调用方看不出来，
  现在多了一个能报告来源的重载）
- 窗口处于最小化
- 有 `WS_THICKFRAME` 却量出全 0 内缩。这台机器上所有 THICKFRAME 窗口都是 8,0,8,8，
  而没有 THICKFRAME 的（CoreWindow、Progman、GTK 的 CSD 窗口）本来就是 0——
  所以这一条不会误伤 Czkawka 那类自绘装饰窗口。

代价是这类窗口每次几何更新多一次 DWM 查询。实测该调用比 `GetWindowRect` 贵约 28µs，
只发生在异常状态且会自愈，正常窗口仍然走缓存。

### 修复：应用从托盘弹出的迷你面板会被描边

Claude 点击托盘图标弹出的快捷输入框，窗口矩形 767x595，而只有顶部一条是不透明的——
结果套了一个几倍于可见内容的边框，还多出一个书签页签。

判据要同时满足四条，缺一不可：

```
无 WS_CAPTION 且无 WS_THICKFRAME    无框，但很多正常窗口也无框
无 WS_SYSMENU                       YeImageViewer、Picasa 无框但都有系统菜单
WS_EX_NOREDIRECTIONBITMAP           纯 DirectComposition 渲染，没有重定向表面
```

实测对照：

| 窗口 | CAPTION | THICKFRAME | SYSMENU | NOREDIR |
|---|---|---|---|---|
| Claude 迷你框 | ✗ | ✗ | ✗ | **✓** |
| Czkawka（GTK） | ✗ | ✗ | ✗ | ✗ |
| YeImageViewer 主窗 | ✗ | ✗ | ✓ | ✗ |
...
**特意没有用 `WS_EX_TOPMOST`**，虽然它同样能区分：置顶功能自己会给窗口加上这个位，
拿它当排除条件的话，用户一置顶 Czkawka 这种无框窗口，它就会掉出跟踪、连带自己的置顶
一起失效。`WS_EX_NOREDIRECTIONBITMAP` 我们从不设置，没有这个反噬。
