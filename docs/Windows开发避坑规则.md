...
### 缓存窗口几何：别把「还没就绪」的答案存下来

- `DwmGetWindowAttribute(DWMWA_EXTENDED_FRAME_BOUNDS)` 在窗口刚创建、或刚从最小化还原时，
  可能返回等同于 `GetWindowRect` 的值。把这个差值缓存起来，就等于永久记住了一个错的内缩量。
- 症状很有欺骗性：边框稳定地偏出 8px，**改一下窗口大小就好了**——因为尺寸变化才触发重算。
  用户描述成「没有刷新过来」。
- 三条不可信的信号：DWM 调用失败、窗口最小化中、**有 `WS_THICKFRAME` 却量出全 0 内缩**。
  最后一条有实测支撑：所有 THICKFRAME 窗口都是 8,0,8,8，没有 THICKFRAME 的才是 0，
  所以它不会误伤 GTK 那种自绘装饰窗口。
- 封装成 `ExtendedFrame(hwnd)` 这种「失败就静默退回」的函数时，**要留一个能报告来源的重载**，
  否则调用方永远不知道自己拿到的是真答案还是兜底值。

### 排除覆盖层：别用自己也会设置的样式位

- 应用从托盘弹出的迷你面板（Claude 的快捷输入框）：窗口矩形远大于可见内容，套上边框后
  大得离谱。判据是**无 CAPTION + 无 THICKFRAME + 无 SYSMENU + `WS_EX_NOREDIRECTIONBITMAP`**，
  四条缺一不可——无框的正常窗口很多（YeImageViewer、Picasa 都无框但有系统菜单）。
- **`WS_EX_TOPMOST` 同样能区分，但绝对不能用**：置顶功能自己会给窗口加上这个位。
  一旦拿它当排除条件，用户置顶一个无框窗口，那个窗口就会掉出跟踪、连带置顶一起失效。
  选判据时要先问一句：**这个位我自己会不会设？**
...
- **「日志里没有」不等于「没发生」**。验证一个「不该出现」的东西时，要做**正面确认**：
  证明那个窗口此刻确实存在，且确实没有被处理。只看日志是空的，和用户根本没复现，
  长得一模一样——这个坑我踩过两次。
