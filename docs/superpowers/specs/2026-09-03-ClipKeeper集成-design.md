...
顶层三个功能项的对勾是本次之前刚加的**只读状态显示**（`MF_POPUP` 项没有命令 ID，点击即
展开子菜单，无法兼作开关）。「剪贴板守护」**没有子菜单**，因此它是唯一能真正点击切换的
——正好符合「点一下开面板，再点一下关」。
...
**宽度实测只多 3px**（187px → 190px，DPI 120）。菜单宽度只由最宽的标签决定，而顶层刚去掉
了灰色 header「WindowMark」——十个拉丁字符原本撑到 212px。所以不必为宽度把标签缩成
「剪贴板」。测量方法与完整数据见 `docs/Windows开发避坑规则.md`。
